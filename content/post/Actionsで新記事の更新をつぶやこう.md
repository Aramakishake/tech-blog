...
Copilotさんに泣きついた結果、以下のスクリプトを最初に挿入することで解決した。
```
- name: Configure Git
  run: |
...
- `$POST_TITLE`に記事タイトル
- `$POST_URL`に記事URL
が代入されて、コピペするだけで投稿可能っちゅー寸法だ。

# おわりに
## まとめ
今回は、Actionsを用いて記事の更新に際してXで自動投稿を行うようなスクリプトの書き方を紹介した。

しかし、出費をケチり、Post用メッセージを作成することに甘んじていることも紹介した。

## 所感
カスのSNSをつけあがらせないために、プログラマーの美徳の1つである怠惰を黙らせた。

この振る舞いに関して弁明をしておく。

というのも、昨今のX、かなり醜悪だと感じいるためだ。特におすすめ欄がひどい。見ててイライラするポストが濁流のように流れてくる。

おそらく、ポストの閲覧を最大化していった結果、人を怒らすことが最もコスパの良い手法だと見つかったためだろうと思われる。

人間の感情をハックして金を稼いでやろうという者共の懐に金を入れたくないという思想から上記の振る舞いをするに至った。

SNSはほどほどで人生をやろうと思う(できるとは言ってない)。
